...
import ru.itmo.wp.web.exception.UnobviousMethodException;
...
public class FrontServlet extends HttpServlet {
    ...
    private void process(HttpServletRequest request, HttpServletResponse response)
        ...
        } catch (NotFoundException | UnobviousMethodException e) {
            try {
                process(Route.newNotFoundRoute(), request, response);
            } catch (NotFoundException | UnobviousMethodException exception) {
    ...
    private void process(Route route, HttpServletRequest request, HttpServletResponse response)
            throws NotFoundException, ServletException, IOException, UnobviousMethodException {
        Class<?> pageClass;
        try {
        ...
        for (Class<?> clazz = pageClass; method == null && clazz != null; clazz = clazz.getSuperclass()) {
            for (Method m : clazz.getDeclaredMethods()) {
                if (m.getName().equals(route.getAction())) {
                    if (method != null) {
                        throw new UnobviousMethodException();
                    }
                    method = m;
                    break;
                }
        ...
        Map<Class<?>, Object> classesToParameters = Map.of(
        ...
        try {
            List<Object> parameters = Arrays.stream(method.getParameters())
                    .map(parameter -> addParameter(parameter, classesToParameters))
    ...
    private static Object addParameter(Parameter parameter, Map<Class<?>, Object> classesToParameters) {
        if (classesToParameters.containsKey(parameter.getType())) {
            return classesToParameters.get(parameter.getType());
        }
        return null;
    }
    ...
    private Template newTemplate(String templateName, Locale locale) throws ServletException {
        ...
        if (sourceConfiguration != null) {
            sourceConfiguration.setLocale(locale);
            try {
        ...
        if (template == null && targetConfiguration != null) {
            targetConfiguration.setLocale(locale);
            try {
