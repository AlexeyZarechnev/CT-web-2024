...
public class FreemarkerServlet extends HttpServlet {
    ...
    private void processTemplate(Template template, HttpServletRequest request, HttpServletResponse response) throws IOException{
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        String templatePath = URLDecoder.decode(request.getRequestURI(), UTF_8);
        if (templatePath.isBlank() || templatePath.equals("/")) {
        ...
        Template template;
        try {
            template = freemarkerConfiguration.getTemplate(templatePath + ".ftlh");
        } catch (TemplateNotFoundException ignored) {
            response.setStatus(HttpServletResponse.SC_NOT_FOUND);
            processTemplate(freemarkerConfiguration.getTemplate("misc/404.ftlh"), request, response);
            return;
        }
    ...
    private static final Map<String, Parser<?>> types = Map.of(
        "user_id", Long::parseLong,
        "logged_user_id", Long::parseLong,
        "post_id", Long::parseLong,
        "handle", (String s) -> s
    );
    ...
    private Map<String, Object> getData(HttpServletRequest request) {
        ...
        for (Map.Entry<String, String[]> e : request.getParameterMap().entrySet()) {
            if (e.getValue() != null && e.getValue().length == 1) {
                try {
                    data.put(e.getKey(), types.get(e.getKey()).parse(e.getValue()[0]));
                } catch (Exception ignored) {}
            }
        }

        data.put("currentPage", request.getRequestURI());
